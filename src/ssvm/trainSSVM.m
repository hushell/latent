function [w, diff] = trainOnlineStructSVM(size2train, w_init, find_MVC, param)
%{
...
while (iter < param.max_num_iterations && trigger)
    ...
    for id = considerIDS
        ...
        score = m-w'*X_wo;   
        fprintf('ID: %d, score = %f, num of constraints = %d\n', id, score, n);
        ...
        for ii=1:numel(check_labels)
            ...
            if sum(X_wo - Constraints(:,label_ii)) == 0
                isMVC=0;
                break;
            end
        end    
    ...
    diff(iter) = sum(w - wPrev);
    fprintf('\nSUM: %d IN ITERATION %d\n', diff(iter), iter);
    fprintf('\nCOST: %d IN ITERATION %d\n', cost, iter);

    trigger = 1;
    if abs(diff(iter)) <= 0.000000001
        trigger = 0;
    end
    wPrev = w;
